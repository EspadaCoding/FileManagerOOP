...
{
protected: 
	int32 shiftLeft(int32 x, int32 n);//цикличиский вздвиг влево ,нужен для подсчета хэша,тоесть идет по кругу
...
MD5::MD5(const string p)
{
	md5(p);
}
int32  MD5::shiftLeft(int32 x, int32 n) {
	return (x << n) | (x >> (32 - n));
}
void MD5::addingLenght(vector<byte>& buf, const int64 len) {

	for (auto i = 7; i >= 0; i--)
		buf.push_back(byte((len >> (i * 8)) & 0xFF));//длина в байтах поэтому на 8 умножаем
}
void MD5::addingNulls(vector<byte>& buf) {
	do
		buf.push_back(0);
	while (buf.size() % 64 != 56);//1)512/8=64 в 64 байтах 512 бит,в 56 байтах 448 бит
}
void MD5::initMDbuf() {
	A = 0x67452301;
	B = 0xEFCDAB89;
	...
	D = 0x10325476;
	for (auto i = 0; i < 64; i++)
		T[i] = int32(pow(2, 32) * abs(sin(double(i + 1))));//массив таблицы константов формула из википедия,если у нас число которое выходит за пределы 32 битов он его обрезает до 32

}
void MD5::adding1byte(vector<byte>& buf) {
	buf.push_back(128);   //128 = 1000 0000
}
int MD5::conv512to32(vector<byte>& buf, int32*& words) {
	auto wSize = buf.size();
...
void MD5::calcBlocks(int32 *buf, const int len) {
...
string MD5::hex(int32 num)//превращаем в 16 ричную херню 
{

	char temp[33] = { 0 };
	for (int i = 0; i < 4; i++) {

		sprintf(temp + i * 2, "%02x", num % 256);
		num /= 256;
	}
	return std::string(temp);
}
//главная функция
string MD5::md5(const string msg) {
	int32* blocks;
